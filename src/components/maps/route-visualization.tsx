...
export function RouteVisualization({ map, route, onStationClick }: RouteVisualizationProps) {
  ...
  useEffect(() => {
    ...
    // Draw route path
    if (route.route && route.route.coordinates) {
      const path = route.route.coordinates.map(([lng, lat]: [number, number]) => ({
        lat,
        lng
      }));
    ...
    if (route.chargingStations) {
      route.chargingStations.forEach((station: ChargingStop, index: number) => {
  ...
  const createStationInfoContent = (station: ChargingStop, stopNumber: number) => {
    return `
      <div style="padding: 8px; min-width: 200px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 4px; font-size: 14px;">
          ...
          <div>💰 Cost: $${station.cost.toFixed(2)}</div>
